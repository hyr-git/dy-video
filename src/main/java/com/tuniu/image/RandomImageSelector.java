...
public class RandomImageSelector {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(!file.exists()){
            throw new ExportException("d://test.txt 源文件不存在");
        }
    ...
    public static void generateNewFolders(String sourceDirPath,String outDirPath,String newDirFileName,int newViewFolders, double repeatRate) throws IOException {
        ...
        RandomImageSelector randomImageSelector = new RandomImageSelector();
        ...
        for (Map.Entry<String, List<String>> entry : newFolderPictureMap.entrySet()) {
            ...
            randomImageSelector.createBatchRandomImageFolders(randomImageFolders,sourceDirPath,outDirPath,newViewFolderName);
        ...
        randomImageFolders.stream().forEach(item->{
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    ;
                    //新的文件路径
                    File newFile = new File(outDirPath + "\\" + newViewFolderName + "\\" , String.format("%03d", number)+"-"+tempImageName);
                    //文件复制
                    try {
                        FileCopyUtils.copy(oldFile, newFile);
                    } catch (IOException e) {
